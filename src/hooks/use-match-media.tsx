...
const useMatchMedia = (mediaQuery: string): boolean => {
  ...
  const [matches, setMatches] = useState<boolean>(() => (mediaQueryList ? mediaQueryList.matches : false));

  useEffect(() => {
    if (mediaQueryList === null) return;
    setMatches(mediaQueryList.matches);
    const listener = (event_: MediaQueryListEventMap["change"]) => setMatches(event_.matches);
